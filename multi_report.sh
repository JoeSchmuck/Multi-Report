	...
	if [[ $softver != "Linux" ]]; then
		if ! [[ "$(find /usr/ -name "gdisk")" ]]; then
			...
			if ! test -e "/tmp/Multi-Report"; then
				...
				echo "/tmp/Multi-Report present, files should be on the system."
			fi

			cp /tmp/Multi-Report/gdisk /usr/local/sbin
			cp /tmp/Multi-Report/sgdisk /usr/local/sbin
...
update_config_file () {
	...
	(
		# Lets convert any old variables into new variables from version 2.4 to 3.0.4
...
if ! [[ "$1" == "-config" || "$1" == "-help" || "$1" == "-delete" || "$1" == "-s" || "$1" == "" || "$1" == "-dump" || "$1" == "-purge" || "$1" == "-h" || "$1" == "-t" || "$1" == "-m" || "$1" == "-u7zip" || $1 == "-update" || "$1" == "-ignore_lock" || "$1" == "-cleanup" || "$1" == "-scsismart" || "$1" == "-smr_update" || "$1" == "-check_smr"  || "$1" == "-disable_smr" || "$1" == "-enable_smr" || "$1" == "-disable_smr_alarm" || "$1" == "-enable_smr_alarm" ]]; then
