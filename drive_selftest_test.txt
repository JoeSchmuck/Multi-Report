...
#
# - Added checking for SCRUB and RESILVER time remaining, if exceeds time limit, then a SHORT test vice LONG test will occur. (3 hour default)
# - Improved 'demo' Mode
# ######################################- Updated Command Line Options - FINISH THIS UP, LIMIT THE DEMO OPTIONS - DEMO SHORT[LONG] - MODE 1[2][3] - PERIOD WEEK[MONTH]
# - Removed erronious 'Month' messages
...
Use_multi_report_config_values="false"      # A "true" value here will use the $Config_File_Name file values to override the values defined below.
                                            # This allows the values to be retained between versions.  A "false" will utilize the values below.

###### SCRIPT UPDATES
Check_For_Updates="true"                    # This will check to see if an update is available. Default="true"
Automatic_Selftest_Update="false"           # WARNING !!!  This option will automatically update the Drive_Selftest script if a newer version exists on GitHub
                                            #  with no user interaction. Default = "false"
...
Long_Drives_Test_Period="Month"             # "Week" (7 days) or "Month" (28 days)
...
Version="1.04"                            # Current version of the script
Version_Date="(3 February 2025)"
...
checkforupdate () {
    ...
    if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
        (
            ...
            cd Multi-Report
####################################################################################################################################################        
####### REMOVE THE _TEST FROM THE THREE FILES NAMES BEFORE ISSUING THE SCRIPT ######################################################################
####################################################################################################################################################

####################################################################################################################################################        
####### REMOVE THE _TEST FROM THE THREE FILES NAMES BEFORE ISSUING THE SCRIPT ######################################################################
####################################################################################################################################################
            curl -LJOs https://raw.githubusercontent.com/JoeSchmuck/Multi-Report/refs/heads/main/drive_selftest_test.txt            
        )    
        if test -e "/tmp/Multi-Report/drive_selftest_test.txt"; then
            GitDriveVersion=$(cat "/tmp/Multi-Report/drive_selftest_test.txt" | grep "##### Version" | cut -d 'n' -f2 | tr -d ' ' | head -1)
            ...
            if test -e $SCRIPT_DIR"/drive_selftest_test.sh"; then
                LocalDriveFilename="$(cat $SCRIPT_DIR"/drive_selftest_test.sh" | grep "##### Version" | cut -d 'n' -f2 | tr -d ' ' | head -1)"
...
####### REMOVE THE _TEST FROM THE THREE FILES NAMES BEFORE ISSUING THE SCRIPT ######################################################################
####################################################################################################################################################
                rm /tmp/Multi-Report/drive_selftest_test.txt
...
####################################################################################################################################################
    ...
    if [[ $UpdateDriveAvailable == "true" ]]; then
        ...
        echo "#                       Or turn on Automatic Updates                           #"
        echo "################################################################################"        
    fi
    echo " "
...
####                                                                                                                                            ####    
        curl -LJOs https://raw.githubusercontent.com/JoeSchmuck/Multi-Report/refs/heads/main/drive_selftest_test.txt
        curl -LJOs https://raw.githubusercontent.com/JoeSchmuck/Multi-Report/refs/heads/main/Drive_Selftest_User_Guide_test.pdf
        )
        GitDriveVersion=$(cat "/tmp/Multi-Report/drive_selftest_test.txt" | grep "##### Version" | cut -d 'n' -f2 | tr -d ' ' | head -1)
####                                                                                                                                            ####
####                                                                                                                                            ####
...
check_scrub () {
    ...
    # If greater than $SCRUB_RESILVER_Minutes_Remaining then we will not run a SMART Long test, we substitute it wiht a Short test.

    Function_In_Progress="false"
    for i in {0..1000}; do            # ARE WE REALLY GOING TO SEE 1000 POOLS?  THIS IS NOT THE ID NUMBER
...
# Local Variables: s_drives, short_modified_drives
    ...
    if ! [[ $selftest_drives == "" ]]; then
        if [[ $Silent != "true" ]]; then
            if [[ $Function_In_Progress == "true" ]]; then
                echo "RUNNING SHORT TEST: "$selftest_drives", '"$Moved_Long_Scrub"' moved from LONG TEST due to SCRUB." | tee -a /tmp/drive_test_temp.txt
            else
...
troubleshooting_help () {
    ...
    echo "Hum, can't be anything else, can it."
    echo " "
    echo "Press Any Key to Continue"
    read -s -n 1 key_input
    echo "--------------------------------------------------------------------------"
...
#if [[ $SHLVL -eq 1 ]]; then
#    checkforupdate
#    Check_For_Updates="false"    # Turn off so we do not do this check more than once.
#fi
...
check_scrub                           # Is a SCRUB going on?
...
if [[ $Enable_Logging == "true" ]]; then
    if [[ $Demo == "true" ]]; then
        ...
        if [[ $Silent != "true" ]]; then
            ...
            echo "Saving Log (file name: $LOG_DIR/drive_test_${today_day}.txt)" >> /tmp/drive_test_temp.txt
            echo "Exiting Joe's SMART Drive Self-test Script" >> /tmp/drive_test_temp.txt
            cat /tmp/drive_test_temp.txt > $LOG_DIR/drive_selftest_${today_day#0}.txt
        fi
    fi
