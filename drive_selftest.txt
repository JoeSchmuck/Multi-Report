...
update_script () {
# NOT OPERATIONAL NOR CALLED IN THE PROCEDURE (YET) - PLACEHOLDER

# This will check for the file 'drive_selftest.up', if the file exists then read the first line for the version number.
# If the version number is greater than the current version, then update script, and delete the 'drive_selftest.up' file.
	Update_GO=0
	
	
# REMOVE THIS BEFORE FINALIZING
cp /mnt/farm2/scripts/drive_selftest.up /tmp/Multi-Report/drive_selftest.up

	# Test for the file.
	if [ -f "/tmp/Multi-Report/drive_selftest.up" ]; then
		echo "Found Drive_Selftest Update Script, checking if newer version..."
		Update_Major_Version=$(head -n 1 /tmp/Multi-Report/drive_selftest.up | grep -m 1 "Version" | cut -d 'V' -f2 | cut -d '.' -f1)
		Update_Minor_Version=$(head -n 1 /tmp/Multi-Report/drive_selftest.up | grep -m 1 "Version" | cut -d 'V' -f2 | cut -d '.' -f2)
		Current_Major_Version=$(echo $Version | cut -d '.' -f1)
		Current_Minor_Version=$(echo $Version | cut -d '.' -f2)		
		if [[ $Update_Major_Version -gt $Current_Major_Version ]]; then 			# Major Version is newer
			Update_GO=1
		elif [[ $Update_Major_Version -ge $Current_Major_Version ]] && [[ $Update_Minor_Version -gt $Current_Minor_Version ]]; then
			Update_GO=1
		else
			echo "No new update."			# Delete this line and the "else" above it.
		fi
	fi
	
	if [[ $Update_GO == 1 ]] && [[ $Allow_Script_Update == "true" ]]; then			# We have an update
		echo "We have an update to the 'drive_selftest.sh' script."
		# ARG!  I need to transfer the variables.
		# READ THE NEW UPDATED FILE AND CHANGE THE MATCHING LINES FOR SPECIFIC VARIABLES.
		
		# VARIABLES TO CHANGE
#		sed -i -e '/*Allow_Script_Update=*/Allow_Script_Update="$Allow_Script_Update" drive_selftest.up
#		sed -i -e 's/Allow_Script_Update=".*/Allow_Script_UpDate="${Test_Data}"/' drive_selftest.up
		Short_Test_Mode=2
		Short_Time_Delay_Between_Drives=1
...
#		Short_Drives_Test_Delay=130
		Long_Test_Mode=1
		Long_Time_Delay_Between_Drives=1
		Long_SMART_Testing_Order="Serial"
		Long_Drives_to_Test_Per_Day=1
		Long_Drives_Test_Period="Week"
		Long_Drives_Tested_Days_of_the_Week="1,2,3,4,5,6,7"
		Ignore_Drives_List="RQTY4D78E,JJ6XTZ,OU812,ZR13JRL"
		Drive_List_Length=10
		Enable_Logging="true"

	else
		echo "Automatic updates are not functional yet.  Run 'drive_selftest.sh -update' to update the script."
	fi
...
help_text () {
	...
	echo " "
    echo "Automatic Updates is Not Functional in this release."
	echo " "
...
if [[ $1 == "-use_external_file" ]]; then Use_multi_report_config_values="true"; fi

if [[ $Use_multi_report_config_values == "true" ]]; then
	if test -e "$Config_File_Name"; then
		. "$Config_File_Name"
#		echo "External File was read..."
	else
		echo "External File Does Not Exist ($Config_File_Name)"
